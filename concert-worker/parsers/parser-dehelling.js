...
const DE_HELLING_VENUE_ID = 3;
const DE_HELLING_CITY = "Utrecht";
const DE_HELLING_SLUG = "De-Helling";

export async function fetchDeHellingEvents(options = {}) {
  const {
    maxPages = 80,
...
function buildAgendaUrl(page = 1) {
  if (page <= 1) {
    return `${PODIUMINFO_BASE}/podium/${DE_HELLING_VENUE_ID}/concerten/${DE_HELLING_SLUG}/${DE_HELLING_CITY}/`;
  }

  return `${PODIUMINFO_BASE}/podium/${DE_HELLING_VENUE_ID}/concerten/${page}/${DE_HELLING_SLUG}/${DE_HELLING_CITY}/`;
}
...
function normalizeMusicEvent(json, nowTs) {
  ...
  const venueNameRaw = clean(json?.location?.name || "De Helling");
...
function extractArtistFromName(name, venueName) {
  ...
  if (venueName) {
    const escapedVenue = escapeRegExp(
      venueName.replace(/^De Helling\s*-\s*/i, "").trim()
    );
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "De Helling";
  if (/^de helling$/i.test(t)) return "De Helling";
  if (/^helling$/i.test(t)) return "De Helling";
  return t;
}
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `dehelling-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
