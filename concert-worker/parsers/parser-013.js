...
function normalizeMusicEvent(json, nowTs) {
  ...
  const venueNameRaw = clean(json?.location?.name || "013");
...
function extractArtistFromName(name, venueName) {
  ...
  if (venueName) {
    const strippedVenue = venueName.replace(/^013\s*-\s*/i, "").trim();
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "013";
  if (/^013$/i.test(t)) return "013";
  if (/^main$/i.test(t)) return "013 - Main";
  if (/^next$/i.test(t)) return "013 - Next";
  if (/^alle zalen$/i.test(t)) return "013 - Alle Zalen";
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `013-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
