...
const MELKWEG_VENUE_ID = 1;
const MELKWEG_CITY = "Amsterdam";
const MELKWEG_SLUG = "Melkweg";

export async function fetchMelkwegEvents(options = {}) {
  const {
    maxPages = 80,
...
function buildAgendaUrl(page = 1) {
  return `${PODIUMINFO_BASE}/podium/${MELKWEG_VENUE_ID}/concerten/${page}/${MELKWEG_SLUG}/${MELKWEG_CITY}/`;
}
...
function normalizeMusicEvent(json, nowTs) {
  ...
  const venueNameRaw = clean(json?.location?.name || "Melkweg");
...
function extractArtistFromName(name, venueName) {
  ...
  if (venueName) {
    const escapedVenue = escapeRegExp(
      venueName.replace(/^Melkweg\s*-\s*/i, "").trim()
    );
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "Melkweg";
  if (/^melkweg$/i.test(t)) return "Melkweg";
  if (/^max$/i.test(t)) return "Melkweg - Max";
  if (/^the max$/i.test(t)) return "Melkweg - Max";
  if (/^oz$/i.test(t)) return "Melkweg - OZ";
  if (/^the oz$/i.test(t)) return "Melkweg - OZ";
  if (/^up$/i.test(t)) return "Melkweg - UP";
  if (/^expo$/i.test(t)) return "Melkweg - Expo";
  if (/^cinema$/i.test(t)) return "Melkweg - Cinema";
  if (/^oude zaal$/i.test(t)) return "Melkweg - Oude Zaal";
  if (/^rabozaal$/i.test(t)) return "Melkweg - Rabozaal";
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `melkweg-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
