...
const VENUE_ID = 38;
const VENUE_CITY = "Zoetermeer";
const VENUE_SLUG = "Boerderij";

export async function fetchBoerderijEvents(options = {}) {
  const {
    maxPages = 20,
...
function normalizeMusicEvent(json, nowTs) {
  ...
  const venueNameRaw = clean(json?.location?.name || "Boerderij");
  ...
  return {
    ...
    city: extractCity(json) || "Zoetermeer",
...
function extractArtistFromName(name, venueName) {
  ...
  if (venueName) {
    const escapedVenue = escapeRegExp(
      venueName.replace(/^Boerderij\s*-\s*/i, "").trim()
    );
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "Boerderij";
  if (/^boerderij$/i.test(t)) return "Boerderij";
  if (/^creativecolors zaal$/i.test(t)) return "Boerderij - CreativeColors Zaal";
  if (/^podium cafe$/i.test(t)) return "Boerderij - Podium Café";
  if (/^podium café$/i.test(t)) return "Boerderij - Podium Café";
  if (/^cafe$/i.test(t)) return "Boerderij - Café";
  if (/^café$/i.test(t)) return "Boerderij - Café";
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `boerderij-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
