...
const VENUE_ID = 19;
const VENUE_CITY = "Groningen";
const VENUE_SLUG = "Vera";

export async function fetchVeraEvents(options = {}) {
  const {
    maxPages = 20,
...
function normalizeMusicEvent(json, nowTs) {
  ...
  const venueNameRaw = clean(json?.location?.name || "Vera");
  ...
  return {
    ...
    city: extractCity(json) || "Groningen",
...
function extractArtistFromName(name, venueName) {
  ...
  if (venueName) {
    const escapedVenue = escapeRegExp(
      venueName.replace(/^Vera\s*-\s*/i, "").trim()
    );
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "Vera";
  if (/^vera$/i.test(t)) return "Vera";
  if (/^main hall$/i.test(t)) return "Vera - Main Hall";
  if (/^kelderbar$/i.test(t)) return "Vera - Kelderbar";
  if (/^cafe$/i.test(t)) return "Vera - Café";
  if (/^café$/i.test(t)) return "Vera - Café";
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `vera-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
