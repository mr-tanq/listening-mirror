...
const EFFENAAR_VENUE_ID = 28;
const EFFENAAR_CITY = "Eindhoven";
const EFFENAAR_SLUG = "Effenaar";

export async function fetchEffenaarEvents(options = {}) {
  const {
    maxPages = 20,
...
function buildAgendaUrl(page = 1) {
  return `${PODIUMINFO_BASE}/podium/${EFFENAAR_VENUE_ID}/concerten/${page}/${EFFENAAR_SLUG}/${EFFENAAR_CITY}/`;
}
...
function normalizeMusicEvent(json, nowTs) {
  ...
  if (start.timestamp < startOfTodayLocal(nowTs, "Europe/Amsterdam")) {
    return null;
  }

  const venueNameRaw = clean(json?.location?.name || "Effenaar");
  ...
  return {
    ...
    city: extractCity(json) || "Eindhoven",
...
function extractArtistFromName(name, venueName) {
  ...
  if (venueName) {
    const escapedVenue = escapeRegExp(
      venueName.replace(/^Effenaar\s*-\s*/i, "").trim()
    );
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "Effenaar";
  if (/^effenaar$/i.test(t)) return "Effenaar";
  if (/^grote zaal$/i.test(t)) return "Effenaar - Grote Zaal";
  if (/^kleine zaal$/i.test(t)) return "Effenaar - Kleine Zaal";
  if (/^main hall$/i.test(t)) return "Effenaar - Grote Zaal";
  if (/^small hall$/i.test(t)) return "Effenaar - Kleine Zaal";
  if (/^cafe$/i.test(t)) return "Effenaar - Cafe";
...
function parseStartDate(value) {
  ...
  const date_local = formatLocalDate(d, "Europe/Amsterdam");
  const time_local = formatLocalTime(d, "Europe/Amsterdam");
...
function startOfTodayLocal(nowTs, timeZone) {
  ...
  const midnightLocal = new Date(`${todayLocal}T00:00:00+01:00`);
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `effenaar-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
