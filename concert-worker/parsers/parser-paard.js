...
const VENUE_ID = 37;
const VENUE_CITY = "Den-Haag";
const VENUE_SLUG = "Paard";
...
function normalizeMusicEvent(json, nowTs) {
  ...
  const venueNameRaw = clean(json?.location?.name || "Paard");
  ...
  return {
    ...
    city: extractCity(json) || "Den Haag",
...
function extractArtistFromName(name, venueName) {
  ...
  if (venueName) {
    const escapedVenue = escapeRegExp(
      venueName.replace(/^Paard\s*-\s*/i, "").trim()
    );
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "Paard";
  if (/^paard$/i.test(t)) return "Paard";
  if (/^grolsch zaal$/i.test(t)) return "Paard - Grolsch Zaal";
  if (/^kleine zaal$/i.test(t)) return "Paard - Kleine Zaal";
  if (/^paardcafe$/i.test(t)) return "Paard - Paardcafé";
  if (/^paardcafé$/i.test(t)) return "Paard - Paardcafé";
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `paard-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
