...
const PATRONAAT_VENUE_ID = 48;
const PATRONAAT_CITY = "Haarlem";
const PATRONAAT_SLUG = "Patronaat";

export async function fetchPatronaatEvents(options = {}) {
  const {
    maxPages = 20,
...
function buildAgendaUrl(page = 1) {
  return `${PODIUMINFO_BASE}/podium/${PATRONAAT_VENUE_ID}/concerten/${page}/${PATRONAAT_SLUG}/${PATRONAAT_CITY}/`;
}
...
function normalizeMusicEvent(json, nowTs) {
  ...
  const venueNameRaw = clean(json?.location?.name || "Patronaat");
...
function extractArtistFromName(name, venueName) {
  ...
  if (venueName) {
    const escapedVenue = escapeRegExp(
      venueName.replace(/^Patronaat\s*-\s*/i, "").trim()
    );
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "Patronaat";
  if (/^patronaat$/i.test(t)) return "Patronaat";
  if (/^grote zaal$/i.test(t)) return "Patronaat - Grote Zaal";
  if (/^kleine zaal$/i.test(t)) return "Patronaat - Kleine Zaal";
  if (/^stage 1$/i.test(t)) return "Patronaat - Stage 1";
  if (/^stage 2$/i.test(t)) return "Patronaat - Stage 2";
  if (/^patronaat cafe$/i.test(t)) return "Patronaat - Cafe";
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `patronaat-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
