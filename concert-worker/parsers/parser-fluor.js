...
const VENUE_ID = 3676;
const VENUE_CITY = "Amersfoort";
const VENUE_SLUG = "Fluor";

export async function fetchFluorEvents(options = {}) {
  const {
    maxPages = 20,
...
function normalizeMusicEvent(json, nowTs) {
  ...
  const venueNameRaw = clean(json?.location?.name || "Fluor");
  ...
  return {
    ...
    city: extractCity(json) || "Amersfoort",
...
function extractArtistFromName(name, venueName) {
  ...
  if (venueName) {
    const escapedVenue = escapeRegExp(
      venueName.replace(/^Fluor\s*-\s*/i, "").trim()
    );
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "Fluor";
  if (/^fluor$/i.test(t)) return "Fluor";
  if (/^grote zaal$/i.test(t)) return "Fluor - Grote Zaal";
  if (/^kleine zaal$/i.test(t)) return "Fluor - Kleine Zaal";
  if (/^cafe$/i.test(t)) return "Fluor - Café";
  if (/^café$/i.test(t)) return "Fluor - Café";
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `fluor-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
