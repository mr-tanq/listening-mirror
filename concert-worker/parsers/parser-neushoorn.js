...
const VENUE_ID = 3677;
const VENUE_CITY = "Leeuwarden";
const VENUE_SLUG = "Neushoorn";

export async function fetchNeushoornEvents(options = {}) {
  const {
    maxPages = 20,
...
function normalizeMusicEvent(json, nowTs) {
  ...
  const venueNameRaw = clean(json?.location?.name || "Neushoorn");
  ...
  return {
    ...
    city: extractCity(json) || "Leeuwarden",
...
function extractArtistFromName(name, venueName) {
  ...
  if (venueName) {
    const escapedVenue = escapeRegExp(
      venueName.replace(/^Neushoorn\s*-\s*/i, "").trim()
    );
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "Neushoorn";
  if (/^neushoorn$/i.test(t)) return "Neushoorn";
  if (/^arena$/i.test(t)) return "Neushoorn - Arena";
  if (/^grote zaal$/i.test(t)) return "Neushoorn - Grote Zaal";
  if (/^cafe$/i.test(t)) return "Neushoorn - Café";
  if (/^café$/i.test(t)) return "Neushoorn - Café";
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `neushoorn-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
