const AFAS_BASE = "https://www.afaslive.nl";
const AFAS_AGENDA_URL = `${AFAS_BASE}/agenda`;

export async function fetchAfasEvents(options = {}) {
  const {
    retries = 2
  } = options;
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    try {
      html = await fetchText(AFAS_AGENDA_URL);
      if (html) break;
    } catch (err) {
      lastError = err;
    }
  ...
  if (!html) {
    throw lastError || new Error("Failed to fetch AFAS Live agenda");
  }

  const agendaItems = parseAgendaPage(html, nowTs);
  const seen = new Set();
  const out = [];

  for (const baseEvent of agendaItems) {
    const key = baseEvent.source_id || makeNormalizedKey(baseEvent);
...
function parseAgendaPage(html, nowTs) {
  ...
  if (!blocks.length) {
    return parseJsonLdFallback(html, nowTs, "afas");
  }

  const results = [];

  for (const item of blocks) {
    const ev = normalizeAfasItem(item, nowTs);
...
function normalizeAfasItem(item, nowTs) {
  ...
  const supports = extractSupportActs(item);
  const title = extractArtistFromTitle(rawTitle) || rawTitle;
  const artistsAll = uniqueStrings([title, ...supports]);
...
function normalizeJsonLdMusicEvent(json, nowTs, sourceName) {
  ...
  const venueName = clean(json?.location?.name || (sourceName === "afas" ? "AFAS Live" : "Ziggo Dome")) ||
    (sourceName === "afas" ? "AFAS Live" : "Ziggo Dome");
...
function absoluteUrl(url) {
  ...
  if (u.startsWith("/")) return `${AFAS_BASE}${u}`;
  return `${AFAS_BASE}/${u}`;
}
