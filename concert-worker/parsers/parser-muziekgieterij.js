...
const VENUE_ID = 859;
const VENUE_CITY = "Maastricht";
const VENUE_SLUG = "Muziekgieterij";

export async function fetchMuziekgieterijEvents(options = {}) {
  const {
    maxPages = 20,
...
function normalizeMusicEvent(json, nowTs) {
  ...
  const venueNameRaw = clean(json?.location?.name || "Muziekgieterij");
  ...
  return {
    ...
    city: extractCity(json) || "Maastricht",
...
function extractArtistFromName(name, venueName) {
  ...
  if (venueName) {
    const escapedVenue = escapeRegExp(
      venueName.replace(/^Muziekgieterij\s*-\s*/i, "").trim()
    );
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "Muziekgieterij";
  if (/^muziekgieterij$/i.test(t)) return "Muziekgieterij";
  if (/^grote zaal$/i.test(t)) return "Muziekgieterij - Grote Zaal";
  if (/^kleine zaal$/i.test(t)) return "Muziekgieterij - Kleine Zaal";
  if (/^cafe$/i.test(t)) return "Muziekgieterij - Café";
  if (/^café$/i.test(t)) return "Muziekgieterij - Café";
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `muziekgieterij-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
