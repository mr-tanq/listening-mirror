...
const TIVOLI_VENUE_ID = 3071;
const TIVOLI_CITY = "Utrecht";
const TIVOLI_SLUG = "TivoliVredenburg";
...
function buildAgendaUrl(page = 1) {
  return `${PODIUMINFO_BASE}/podium/${TIVOLI_VENUE_ID}/concerten/${page}/${TIVOLI_SLUG}/${TIVOLI_CITY}/`;
}
...
function normalizeMusicEvent(json, nowTs) {
  ...
  const venueNameRaw = clean(json?.location?.name || "TivoliVredenburg");
...
function extractArtistFromName(name, venueName) {
  ...
  if (venueName) {
    const escapedVenue = escapeRegExp(
      venueName.replace(/^TivoliVredenburg\s*-\s*/i, "").trim()
    );
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "TivoliVredenburg";
  if (/^tivolivredenburg$/i.test(t)) return "TivoliVredenburg";
  if (/^ronda$/i.test(t)) return "TivoliVredenburg - Ronda";
  if (/^pandora$/i.test(t)) return "TivoliVredenburg - Pandora";
  if (/^grote zaal$/i.test(t)) return "TivoliVredenburg - Grote Zaal";
  if (/^cloud nine$/i.test(t)) return "TivoliVredenburg - Cloud Nine";
  if (/^club nine$/i.test(t)) return "TivoliVredenburg - Club Nine";
  if (/^hertz$/i.test(t)) return "TivoliVredenburg - Hertz";
  if (/^rabo open stage$/i.test(t)) return "TivoliVredenburg - Rabo Open Stage";
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `tivoli-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
