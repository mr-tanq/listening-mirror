...
const VENUE_SLUG = "dBs";

export async function fetchDbsEvents(options = {}) {
  const {
    maxPages = 20,
...
function normalizeMusicEvent(json, nowTs) {
  ...
  const venueNameRaw = clean(json?.location?.name || "dB's");
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "dB's";
  if (/^db'?s$/i.test(t)) return "dB's";
  if (/^grote zaal$/i.test(t)) return "dB's - Grote Zaal";
  if (/^zaal$/i.test(t)) return "dB's - Zaal";
  if (/^cafe$/i.test(t)) return "dB's - Café";
  if (/^café$/i.test(t)) return "dB's - Café";
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `dbs-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
