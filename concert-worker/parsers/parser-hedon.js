...
const VENUE_ID = 22;
const VENUE_CITY = "Zwolle";
const VENUE_SLUG = "Hedon";

export async function fetchHedonEvents(options = {}) {
  const {
    maxPages = 20,
...
function normalizeMusicEvent(json, nowTs) {
  ...
  const venueNameRaw = clean(json?.location?.name || "Hedon");
...
function extractArtistFromName(name, venueName) {
  ...
  if (venueName) {
    const escapedVenue = escapeRegExp(
      venueName.replace(/^Hedon\s*-\s*/i, "").trim()
    );
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "Hedon";
  if (/^hedon$/i.test(t)) return "Hedon";
  if (/^grote zaal$/i.test(t)) return "Hedon - Grote Zaal";
  if (/^small hall$/i.test(t)) return "Hedon - Small Hall";
  if (/^foyer$/i.test(t)) return "Hedon - Foyer";
  if (/^cafe$/i.test(t)) return "Hedon - Café";
  if (/^café$/i.test(t)) return "Hedon - Café";
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `hedon-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
