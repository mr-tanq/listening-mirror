const AFAS_BASE = "https://www.podiuminfo.nl";
const AFAS_VENUE_ID = 4259;
const AFAS_CITY = "Amsterdam";
const AFAS_SLUG = "AFAS-Live";
...
function buildAgendaUrl(page = 1) {
  return `${AFAS_BASE}/podium/${AFAS_VENUE_ID}/concerten/${page}/${AFAS_SLUG}/${AFAS_CITY}/`;
}
...
function normalizeMusicEvent(json, nowTs) {
  ...
  const venueNameRaw = clean(json?.location?.name || "AFAS Live");
...
function extractArtistFromName(name, venueName) {
  ...
  if (venueName) {
    const escapedVenue = escapeRegExp(
      venueName.replace(/^AFAS Live\s*-\s*/i, "").trim()
    );
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "AFAS Live";
  if (/^afas live$/i.test(t)) return "AFAS Live";
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `afas-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
...
function absoluteUrl(url) {
  ...
  if (url.startsWith("/")) return `${AFAS_BASE}${url}`;
  return `${AFAS_BASE}/${url}`;
}
