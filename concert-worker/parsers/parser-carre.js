...
const VENUE_ID = 34;
const VENUE_CITY = "Amsterdam";
const VENUE_SLUG = "Koninklijk-Theater-Carre";

export async function fetchCarreEvents(options = {}) {
  const {
    maxPages = 20,
...
function normalizeMusicEvent(json, nowTs) {
  ...
  const venueNameRaw = clean(json?.location?.name || "Koninklijk Theater Carré");
...
function extractArtistFromName(name, venueName) {
  ...
  if (venueName) {
    const strippedVenue = venueName
      .replace(/^Koninklijk Theater Carré\s*-\s*/i, "")
      .replace(/^Koninklijk Theater Carre\s*-\s*/i, "")
      .replace(/^Carré\s*-\s*/i, "")
      .replace(/^Carre\s*-\s*/i, "")
      .trim();

    const escapedVenue = escapeRegExp(strippedVenue);
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "Koninklijk Theater Carré";
  if (/^koninklijk theater carré$/i.test(t)) return "Koninklijk Theater Carré";
  if (/^koninklijk theater carre$/i.test(t)) return "Koninklijk Theater Carré";
  if (/^carré$/i.test(t)) return "Koninklijk Theater Carré";
  if (/^carre$/i.test(t)) return "Koninklijk Theater Carré";
  if (/^grote zaal$/i.test(t)) return "Koninklijk Theater Carré - Grote Zaal";
  if (/^foyer$/i.test(t)) return "Koninklijk Theater Carré - Foyer";
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `carre-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
