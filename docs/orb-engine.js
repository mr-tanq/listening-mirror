/* orb-engine.js (FULL FILE REPLACE)
   Listening Mirror — Clean Neural Orb Engine
   ✅ NO PNG assets
   ✅ Pure canvas
   ✅ Neural strands + particles + glow
   ✅ Stable on mobile
   Exposes: window.LMOrbEngine.mount(canvas, opts)
*/
   ...
   class OrbEngine {
    ...
    makeStrands() {
      ...
      for (let i = 0; i < 18; i++) {
        strands.push({
          a0: rand() * TAU,
          speed: lerp(0.10, 0.42, rand()),
          radiusBias: lerp(0.36, 0.92, rand()),
          width: lerp(0.6, 2.4, rand()),
    ...
    makeParticles() {
      ...
      for (let i = 0; i < 44; i++) {
        pts.push({
          a0: rand() * TAU,
          speed: lerp(-0.28, 0.28, rand()),
          ring: lerp(0.42, 1.04, rand()),
    ...
    destroy() {
      ...
      const g = ctx.createRadialGradient(cx, cy, radius * 0.08, cx, cy, radius * 1.45);
      g.addColorStop(0.00, rgba(this.palette.light, 0.06 + pulse * 0.08));
      g.addColorStop(0.24, rgba(this.palette.warm, 0.10 + this.heat * 0.08));
      g.addColorStop(0.48, rgba(this.palette.cold, 0.10 + this.depth * 0.08));
    ...
    drawCore(ctx, cx, cy, radius, pulse) {
      const core = ctx.createRadialGradient(cx, cy, radius * 0.01, cx, cy, radius);
      core.addColorStop(0.00, rgba("#ffffff", 0.95));
      core.addColorStop(0.05, rgba(this.palette.light, 0.88));
      core.addColorStop(0.16, rgba(this.palette.warm, 0.28 + this.heat * 0.12));
      core.addColorStop(0.32, rgba(this.palette.cold, 0.18 + this.depth * 0.10));
      core.addColorStop(0.52, rgba(this.palette.fusion, 0.08 + this.focus * 0.06));
      core.addColorStop(1.00, "rgba(0,0,0,0)");

      ctx.fillStyle = core;
      ctx.beginPath();
      ctx.arc(cx, cy, radius * (1 + pulse * 0.03), 0, TAU);
      ctx.fill();

      ctx.strokeStyle = rgba(this.palette.light, 0.12 + pulse * 0.20);
      ctx.lineWidth = 1.0 + this.focus * 0.8;
      ctx.beginPath();
      ctx.arc(cx, cy, radius * 0.98, 0, TAU);
      ctx.stroke();
    }

    drawStrands(ctx, cx, cy, radius, t) {
      const fluxBoost = 0.4 + this.flux * 1.0;
      const focusTight = 0.85 - this.focus * 0.18;
      const strandAlpha = 0.08 + this.flux * 0.18 + this.focus * 0.06;
      ...
      for (const s of this.struts) {
        ...
        const r0 = radius * (0.24 + s.radiusBias * 0.16) * focusTight;
        const r1 = radius * (0.72 + s.radiusBias * 0.26);
        const bend = radius * s.bend * (0.7 + this.flux * 0.5);
        ...
        ctx.lineWidth = s.width * (0.8 + this.focus * 0.45);
    ...
    drawHalo(ctx, cx, cy, radius, pulse) {
      ctx.save();
      ctx.globalCompositeOperation = "screen";
      ctx.strokeStyle = rgba(this.palette.cold, 0.08 + this.depth * 0.14 + pulse * 0.05);
      ctx.lineWidth = 2.2 + this.depth * 2.6;
      ctx.beginPath();
      ctx.arc(cx, cy, radius * (1.10 + pulse * 0.01), 0, TAU);
      ctx.stroke();

      ctx.strokeStyle = rgba(this.palette.fusion, 0.04 + this.depth * 0.08);
      ctx.lineWidth = 6 + this.depth * 5;
      ctx.beginPath();
      ...
      const pulse = Math.pow(Math.max(0, 1 - beat), 4.0) * (0.12 + this.beatEnergy * 0.24);

      const radiusBase = Math.min(w, h) * 0.24;
      const radius = radiusBase * (
        1
        + Math.sin(t * (0.7 + this.flux * 0.5)) * 0.012
        + pulse * 0.05
      );
      ...
      this.drawHalo(ctx, cx, cy, radius, pulse);
