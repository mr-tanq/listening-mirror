...
const DOORNROOSJE_VENUE_ID = 47;
const DOORNROOSJE_CITY = "Nijmegen";
const DOORNROOSJE_SLUG = "Doornroosje";
...
function buildAgendaUrl(page = 1) {
  return `${PODIUMINFO_BASE}/podium/${DOORNROOSJE_VENUE_ID}/concerten/${page}/${DOORNROOSJE_SLUG}/${DOORNROOSJE_CITY}/`;
}
...
function normalizeMusicEvent(json, nowTs) {
  ...
  const venueNameRaw = clean(json?.location?.name || "Doornroosje");
  ...
  return {
    ...
    city: extractCity(json) || "Nijmegen",
...
function extractArtistFromName(name, venueName) {
  ...
  if (venueName) {
    const escapedVenue = escapeRegExp(
      venueName.replace(/^Doornroosje\s*-\s*/i, "").trim()
    );
...
function normalizeVenueName(raw) {
  ...
  if (!t) return "Doornroosje";
  if (/^doornroosje$/i.test(t)) return "Doornroosje";
  if (/^grote zaal$/i.test(t)) return "Doornroosje - Grote Zaal";
  if (/^kleine zaal$/i.test(t)) return "Doornroosje - Kleine Zaal";
  if (/^zaal 1$/i.test(t)) return "Doornroosje - Zaal 1";
  if (/^zaal 2$/i.test(t)) return "Doornroosje - Zaal 2";
...
function buildSourceId({ title, dateLocal, venueName }) {
  return `doornroosje-${slugify(title)}-${slugify(venueName)}-${dateLocal}`;
}
